                ...
                for fp, i in zip(jpg_file_path, range(len(jpg_file_path))):
                    ...
                    for num in range(len(num_detections)):
                        ...
                        if py_scores[num] > threshold:
                            ...
                            result_l = [
                                fp, int(py_classes[num]), py_boxes[num], py_scores[num], 0.0]
...
def get_predict_box(info_list):
    ...
    xmin, ymin, xmax, ymax = [int(info_list[0][2][0] * height), int(info_list[0][2]
                                                                    [1] * width), int(info_list[0][2][2] * height), int(info_list[0][2][3] * width)]
...
def sobel_seg(info_list):
    ...
    edges = img[(ymin-w_expanding):(ymax+w_expanding),
                (xmin-l_expanding):(xmax+l_expanding)]
...
def contours_seg(info_list):
    ...
    contours, hierarchy = cv2.findContours(
        binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)    # 输出为三个参数
