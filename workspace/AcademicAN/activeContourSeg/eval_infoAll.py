        ...
        with self.detection_graph.as_default():
            with tf.Session(graph=self.detection_graph) as sess:
                ...
                for fp, i in zip(jpg_file_path, range(len(jpg_file_path))):
                    ...
                    # print(py_classes,"\n","-----")
                    # print(py_boxes,"\n","-----")
                sp_plit = self.PATH_TO_CKPT.split('/')
                np.save(image_path+"/" + sp_plit[-4]+"_"+ sp_plit[-3] + "_" + "detection_info.npy", info_list)
...
if __name__ == "__main__":
    #-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # 横切
    t_r_PATH_TO_CKPT = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Transverse/ResNetTrain/trained-inference-graphs3cls/frozen_inference_graph.pb"
    t_I_PATH_TO_CKPT = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Transverse/InceptionTrain/trained-inference-graphs3cls/frozen_inference_graph.pb"
    t_ir_PATH_TO_CKPT = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Transverse/inceptionResNetTrain/trained-inference-graphs_blood/frozen_inference_graph.pb"
    # 纵切
    l_r_PATH_TO_CKPT = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Longitudinal/ResNetTrain/trained-inference-graphs3cls/frozen_inference_graph.pb"
    l_I_PATH_TO_CKPT = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Longitudinal/InceptionTrain/trained-inference-graphs3cls/frozen_inference_graph.pb"
    l_ir_PATH_TO_CKPT = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Longitudinal/inceptionResNetTrain/trained-inference-vessel/frozen_inference_graph.pb"
    #------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    label_map = {1: "plaque", 2: "sclerosis", 3: "vessel"}
    eval_path = "/home/andy/anaconda3/ANCODE/axjingWorks/workspace/AcademicAN/paper_img/raw_testT"
    threshold=0.1
    detecotr=TOD(t_ir_PATH_TO_CKPT)
    info_list=detecotr.get_detect_info(eval_path, threshold)
    sp_plit = t_ir_PATH_TO_CKPT.split("/")
    det_path = eval_path+ "/" + sp_plit[-4]+"_"+ sp_plit[-3] + "_" + "detection_info.npy"
    annopath = eval_path
    imagesetfile = eval_path
    # classname = "vessel"
    for i in range(1,4):
    # cachedir = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Transverse/InceptionTrain/images/balance_test/xml"
        print(t_ir_PATH_TO_CKPT, ":", label_map[i])
        rec, prec, ap = voc_eval(det_path,
                annopath,
                imagesetfile,
                classname=label_map[i])
    
    detecotr=TOD(t_r_PATH_TO_CKPT)
    info_list=detecotr.get_detect_info(eval_path, threshold)
    sp_plit = t_r_PATH_TO_CKPT.split("/")
    det_path = eval_path+ "/" + sp_plit[-4]+"_"+ sp_plit[-3] + "_" + "detection_info.npy"
    annopath = eval_path
    imagesetfile = eval_path
    # classname = "vessel"
    for i in range(1,4):
        print(t_r_PATH_TO_CKPT, ":", label_map[i])
        rec, prec, ap = voc_eval(det_path,
                annopath,
                imagesetfile,
                classname=label_map[i])
    
    detecotr=TOD(t_I_PATH_TO_CKPT)
    info_list=detecotr.get_detect_info(eval_path, threshold)
    sp_plit = t_I_PATH_TO_CKPT.split("/")
    det_path = eval_path+ "/" + sp_plit[-4]+"_"+ sp_plit[-3] + "_" + "detection_info.npy"
    annopath = eval_path
    imagesetfile = eval_path
    # classname = "vessel"
    for i in range(1,4):
        print(t_I_PATH_TO_CKPT, ":", label_map[i])
        rec, prec, ap = voc_eval(det_path,
                annopath,
                imagesetfile,
                classname=label_map[i])
    ...
    #------------------------------------------------------------------------------------------------------------
    eval_path = "/home/andy/anaconda3/ANCODE/axjingWorks/workspace/AcademicAN/paper_img/raw_testL"
    detecotr=TOD(l_ir_PATH_TO_CKPT)
    info_list=detecotr.get_detect_info(eval_path, threshold)
    sp_plit = l_ir_PATH_TO_CKPT.split("/")
    det_path = eval_path+ "/" + sp_plit[-4]+"_"+ sp_plit[-3] + "_" + "detection_info.npy"
    annopath = eval_path
    imagesetfile = eval_path
    # classname = "vessel"
    for i in range(1,4):
    # cachedir = "/home/andy/anaconda3/envs/tensorflow-gpu/axingWorks/ANcWork/Transverse/InceptionTrain/images/balance_test/xml"
        print(l_ir_PATH_TO_CKPT, ":", label_map[i])
        rec, prec, ap = voc_eval(det_path,
                annopath,
                imagesetfile,
                classname=label_map[i])
    
    detecotr=TOD(l_r_PATH_TO_CKPT)
    info_list=detecotr.get_detect_info(eval_path, threshold)
    sp_plit = l_r_PATH_TO_CKPT.split("/")
    det_path = eval_path+ "/" + sp_plit[-4]+"_"+ sp_plit[-3] + "_" + "detection_info.npy"
    annopath = eval_path
    imagesetfile = eval_path
    # classname = "vessel"
    for i in range(1,4):
        print(l_r_PATH_TO_CKPT, ":", label_map[i])
        rec, prec, ap = voc_eval(det_path,
                annopath,
                imagesetfile,
                classname=label_map[i])
    
    detecotr=TOD(l_I_PATH_TO_CKPT)
    info_list=detecotr.get_detect_info(eval_path, threshold)
    sp_plit = l_I_PATH_TO_CKPT.split("/")
    det_path = eval_path+ "/" + sp_plit[-4]+"_"+ sp_plit[-3] + "_" + "detection_info.npy"
    annopath = eval_path
    imagesetfile = eval_path
    # classname = "vessel"
    for i in range(1,4):
        print(l_I_PATH_TO_CKPT, ":", label_map[i])
        rec, prec, ap = voc_eval(det_path,
                annopath,
                imagesetfile,
                classname=label_map[i])
