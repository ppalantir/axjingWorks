...
1. 访问顶点v；
2. 依次从v的未被访问的邻接点出����，对图进行深度优先遍历；直至图中和v有路径相通的顶点都被访问；
...
关于大疆机器学习岗位的笔试题B卷（分两次考试：A卷和B卷），对其他岗位不具有参考性。今年的大疆笔试题题量大（单选，多选，填空，问答，编程），时间却很短（1个小时），可以说是时间短，题目多。好多人都没有做完，包括我。不说了，趁着刚做完，把题目做一个记录，题目来自于一些同学的拍照和自己的记忆。题目单选题
